...
import com.cy.wx.model.WeixinPhoto;
...
public class POIUtilTest {
	...
	public void testAddPOI(){
		...
		List<WeixinPhoto> phoneLis = new ArrayList<WeixinPhoto>();
		...
		JSONObject jsonObj = JSONObject.fromObject(bussMap);
		JSONObject jsonBuss = jsonObj.getJSONObject("business");
		JSONObject jsonBase = jsonBuss.getJSONObject("base_info");
		WeixinPOI wxPOI = (WeixinPOI)JSONObject.toBean(jsonBase, WeixinPOI.class);
	...
	public void testGetPOILis(){
		List<WeixinPOI> poiLis = new ArrayList<WeixinPOI>();
		int begin = 0;
		int limit = 10;
		...
		if(jsonData != null){
			if(jsonData.getInt("errcode") == 0){
				...
				totalCount = jsonData.getInt("total_count");
			}
		}
