...
import com.cy.wx.model.UserTag;
import com.cy.wx.model.UserTagMember;
import com.cy.wx.model.WeixinUserInfo;
import com.cy.wx.model.WeixinUserList;
...
public class UserUtil {
	...
	public static UserTag createTag(String accessToken, String tagName) {
		...
		if (null != jsonObject) {
			try {
				userTag = new UserTag();
				userTag.setId(jsonObject.getJSONObject("tag").getInt("id"));
				userTag.setName(jsonObject.getJSONObject("tag").getString("name"));
			} catch (JSONException e) {
				userTag = null;
				int errorCode = jsonObject.getInt("errcode");
				String errorMsg = jsonObject.getString("errmsg");
				logger.error("创建标签失败 errcode:{} errmsg:{}", errorCode, errorMsg);
			}
		}
		return userTag;
	}
	...
	public static List<UserTag> getTags(String accessToken) {
		...
		if (null != jsonObject) {
			try {
				userTagList = JSONArray.toList(jsonObject.getJSONArray("tags"), new UserTag(), new JsonConfig()); 
			} catch (JSONException e) {
				userTagList = null;
				int errorCode = jsonObject.getInt("errcode");
				String errorMsg = jsonObject.getString("errmsg");
				logger.error("查询标签失败 errcode:{} errmsg:{}", errorCode, errorMsg);
			}
		}
		return userTagList;
	}
	...
	public static boolean updateTag(String accessToken, int tagId, String tagName) {
		...
		if (null != jsonObject) {
			...
			if (0 == errorCode) {
				result = true;
				logger.info("编辑标签成功 errcode:{} errmsg:{}", errorCode, errorMsg);
			} else {
				logger.error("编辑标签失败 errcode:{} errmsg:{}", errorCode, errorMsg);
			}
		}
		return result;
	}
	...
	public static boolean deleteTag(String accessToken, int tagId) {
		...
		if (null != jsonObject) {
			...
			if (0 == errorCode) {
				result = true;
				logger.info("删除标签成功 errcode:{} errmsg:{}", errorCode, errorMsg);
			} else {
				logger.error("删除标签失败 errcode:{} errmsg:{}", errorCode, errorMsg);
			}
		}
	...
	public static WeixinUserList getTag(String accessToken, int tagId, String nextOpenId) {
		...
		if (null != jsonObject) {
			try {
				...
				weixinUserList.setOpenIdList(JSONArray.toList(dataObject.getJSONArray("openid"), new ArrayList<String>(), new JsonConfig()));
			} catch (JSONException e) {
				weixinUserList = null;
				int errorCode = jsonObject.getInt("errcode");
				String errorMsg = jsonObject.getString("errmsg");
				logger.error("获取标签下粉丝列表失败 errcode:{} errmsg:{}", errorCode, errorMsg);
			}
		}
		return weixinUserList;
	}
	...
	public static boolean batchTagging(String accessToken, int tagId, List<String> openid_list) {
		...
		JSONObject jsonObject = CommonUtil.httpsRequest(requestUrl, "POST", JSONObject.fromObject(userTagMember).toString());
		if (null != jsonObject) {
			int errorCode = jsonObject.getInt("errcode");
			String errorMsg = jsonObject.getString("errmsg");
			if (0 == errorCode) {
				result = true;
				logger.info("批量为用户打标签成功 errcode:{} errmsg:{}", errorCode, errorMsg);
			} else {
				logger.error("批量为用户打标签失败 errcode:{} errmsg:{}", errorCode, errorMsg);
			}
		}
	...
	public static boolean batchUntagging(String accessToken, int tagId, List<String> openid_list) {
		...
		if (null != jsonObject) {
			...
			if (0 == errorCode) {
				result = true;
				logger.info("批量为用户取消标签成功 errcode:{} errmsg:{}", errorCode, errorMsg);
			} else {
				logger.error("批量为用户取消标签失败 errcode:{} errmsg:{}", errorCode, errorMsg);
			}
		}
	...
	public static List<Integer> getTagidList(String accessToken, String openid) {
		...
		if (null != jsonObject) {
			try {
				userTagList = JSONArray.toList(jsonObject.getJSONArray("tagid_list"), new Integer(1), new JsonConfig()); 
			} catch (JSONException e) {
				userTagList = null;
				int errorCode = jsonObject.getInt("errcode");
				String errorMsg = jsonObject.getString("errmsg");
				logger.error("获取用户身上的标签列表 errcode:{} errmsg:{}", errorCode, errorMsg);
			}
		}
		return userTagList;
	}
	...
	public static boolean setUserRemarkName(String accessToken, String openId, String remark) {
		...
		if (null != jsonObject) {
			...
			if (0 == errorCode) {
				...
				logger.error("设置用户备注名失败 errcode:{} errmsg:{}", errorCode, errorMsg);
			}
		}
		return result;
	}
	...
	public static WeixinUserInfo getUserInfo(String accessToken, String openId, String lang) {
		...
		if (null != jsonObject) {
			try {
				...
				// 用户被打上的标签ID列表
				weixinUserInfo.setTagid_list(JSONArray.toList(jsonObject.getJSONArray("tagid_list"), new ArrayList<String>(), new JsonConfig()));
			} catch (Exception e) {
	...
	public static List<WeixinUserInfo> getUserInfoBatch(String accessToken, List<String> openIdLis, String lang) {
		...
		// 获取用户信息
		JSONObject jsonObject = CommonUtil.httpsRequest(requestUrl, "POST", JSONObject.fromObject(map).toString());
		if (null != jsonObject) {
			try {
				wuiLis = JSONArray.toList(jsonObject.getJSONArray("user_info_list"), new WeixinUserInfo(), new JsonConfig());
			} catch (Exception e) {
				int errorCode = jsonObject.getInt("errcode");
				String errorMsg = jsonObject.getString("errmsg");
				logger.error("获取用户信息(批量)失败 errcode:{} errmsg:{}", errorCode, errorMsg);
			}
		}
		return wuiLis;
	}
	...
	public static WeixinUserList getBlackList(String accessToken, String begin_openid) {
		...
		if (null != jsonObject) {
			try {
				...
				int errorCode = jsonObject.getInt("errcode");
				String errorMsg = jsonObject.getString("errmsg");
				logger.error("获取公众号的黑名单列表失败 errcode:{} errmsg:{}", errorCode, errorMsg);
			}
		}
		return weixinUserList;
	}
	...
	public static boolean batchBlackList(String accessToken, List<String> openid_list) {
		...
		if (null != jsonObject) {
			...
			if (0 == errorCode) {
				result = true;
				logger.info("批量拉黑用户成功 errcode:{} errmsg:{}", errorCode, errorMsg);
			} else {
				logger.error("批量拉黑用户失败 errcode:{} errmsg:{}", errorCode, errorMsg);
			}
		}
	...
	public static boolean batchUnblackList(String accessToken, List<String> openid_list) {
		...
		if (null != jsonObject) {
			...
			if (0 == errorCode) {
				result = true;
				logger.info("批量取消拉黑用户成功 errcode:{} errmsg:{}", errorCode, errorMsg);
			} else {
				logger.error("批量取消拉黑用户失败 errcode:{} errmsg:{}", errorCode, errorMsg);
			}
		}
